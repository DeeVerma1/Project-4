...
def bar_plot(df, col):
    ...
    bar_ax.set_title(f'Cases by {df.columns[0].title()}', size=10)
    bar_ax.set_ylabel('Percentage', size=10)
...
def bar_plot_norm(df,col):
    ...
    bar_norm_ax.set_title(f'Cases by {df.columns[0].title()}\n Normalized by Population %', size=10)
    bar_norm_ax.set_ylabel('Normalized Percentage', size=10)
...
st.markdown('<p class="small-font">https://www.bbc.com/news/health-52623141</p>', unsafe_allow_html=True)
